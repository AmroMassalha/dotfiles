...
HEADLINE_AWS_CMD='echo $AWS_PROFILE'
...
HEADLINE_HOST_TO_AWS=' via '
HEADLINE_AWS_TO_PATH=' in '
HEADLINE_PATH_TO_BRANCH=' ─'
HEADLINE_PATH_TO_PAD=' ─'
...
HEADLINE_STYLE_AWS=$bold$green
HEADLINE_STYLE_K8S=$bold$green  # Style for Kubernetes context
...
HEADLINE_STYLE_AWS_LINE=$HEADLINE_STYLE_AWS
...
headline_git_branch() {
  ...
  if [[ $ret == 0 ]]; then
    ...
    echo "$HEADLINE_GIT_HASH$ref" # hash prefixed to distinguish from branch
  fi
}
...
headline_precmd() {
  ...
  # Information
  local user_str host_str aws_str path_str branch_str status_str
  user_str=$(eval $HEADLINE_USER_CMD)
  host_str=$(eval $HEADLINE_HOST_CMD)
  aws_str=$(eval $HEADLINE_AWS_CMD)
  ...
  if (( ${#status_str} )); then
    ...
    if (( $_HEADLINE_LEN_REMAIN < ${#HEADLINE_PAD_TO_BRANCH} + ${#HEADLINE_BRANCH_PREFIX} + ${#HEADLINE_TRUNC_PREFIX} )); then
      user_str=''; host_str=''; aws_str=''; path_str=''; branch_str=''
    fi
  fi
  ...
  # Trimming
  local joint_len=$(( ${#HEADLINE_USER_BEGIN} + ${#HEADLINE_USER_TO_HOST} + ${#HEADLINE_HOST_TO_AWS} + ${#HEADLINE_AWS_TO_PATH} + ${#HEADLINE_PATH_TO_BRANCH} ))
  ...
  if (( $len < 2 )); then
    user_str=''; host_str=''; aws_str=''
  elif (( $len < ${#user_str} + ${#host_str} + ${#aws_str} )); then
  ...
  if (( ${#aws_str} )); then
    if (( ${#_HEADLINE_INFO_LEFT} )); then
      _headline_part JOINT "$HEADLINE_HOST_TO_AWS" left
    fi
    _headline_part AWS "$HEADLINE_AWS_PREFIX$aws_str" left
  fi
  ...
  if (( ${#path_str} )); then
    if (( ${#_HEADLINE_INFO_LEFT} )); then
      if (( ${#aws_str} )); then
        _headline_part JOINT "$HEADLINE_AWS_TO_PATH" left
      else
  ...
  if [[ $HEADLINE_DO_CLOCK == 'true' ]]; then
    RPROMPT='%{$HEADLINE_STYLE_CLOCK%}$(date +$HEADLINE_CLOCK_FORMAT)%{$reset%}'$RPROMPT
  fi
}
...
# Update RPROMPT with Kubernetes context when typing specific commands
update_rprompt_with_k8s_context() {
  # Get the first word of the current command line buffer
  local cmd=${BUFFER%% *}
  # List of Kubernetes-related commands
  local k8s_commands=(kubectl kubectx kubens k helm crossplane)
  # Check if the command matches any in the list
  if [[ " ${k8s_commands[@]} " == *" ${cmd} "* ]]; then
    # Get the current Kubernetes context
    local k8s_context=$(kubectl config current-context 2>/dev/null)
    if [[ -n "$k8s_context" ]]; then
      # Update RPROMPT to include the Kubernetes context
      RPROMPT="%{$HEADLINE_STYLE_K8S%}$k8s_context%{$reset%}$HEADLINE_RPROMPT"
    else
      RPROMPT="$HEADLINE_RPROMPT"
    fi
  else
...
# Hook the function into ZLE
autoload -Uz add-zle-hook-widget
add-zle-hook-widget line-pre-redraw update_rprompt_with_k8s_context
